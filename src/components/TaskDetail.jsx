...
class TaskDetail extends Component {
    ...
    componentDidMount() {
        if (this.state.taskId != null) {
            ...
            console.log(this.state.socket, " PID = ", this.state.projectId, this.state.taskId);
            Tasks.get(this.state.socket, 1, this.state.taskId, (rs)=> {
                if (!rs) {
    ...
    render() {
        ...
        return (
            <div style={{display: this.checkTaskId()}}>
                        ...
                        <div className="col s8">
                            ...
                            <div id="todoList">
                                {this.state.showTodo ?
                                    ...
                                    :
                                    <div className="addSubject" onClick={this.showTodoList.bind(this)}><i
                                        className="material-icons">note_add</i> Add Checklist Itme</div>
                                }

                            </div>
                            <div id="fileList">
                                <div className="addSubject" onClick={this.onOpenClick}><i className="material-icons">note_add</i>
                                    Attachment
                                </div>
                                            ...
                                            <div id={"att-" + at_item.id} className="attachemnt-item"
                                                ...
                                                <a href={"/uploads/attachment/" + at_item["a.file_name"]} target="_blank">
                                                    { at_item["a.file_type"] == "png" || at_item["a.file_type"] == "jpg" || at_item["a.file_type"] == "jpeg" || at_item["a.file_type"] == "gif" ?
                                                        <div className="img-picture img-100"
                                                             style={{backgroundImage: 'url(/uploads/attachment/' + at_item["a.file_name"] + ')'}}></div>
                                                        : <div
                                                        className={"img-file img-100 img-" + at_item["a.file_type"]}></div>
                                                    }
                                                </a>
                                                <div className="attachment-rm circle"
                                                     onClick={this.onAttachmentRemoveClick.bind(this, at_item.id, at_item["a.file_name"])}
                                                     data-id={at_item.id}>x
                                                </div>
                                            </div>
                            ...
                            <div id="activity">
                                ...
                                <div id="inputComment">
                                    <form onSubmit={this.submitComment.bind(this)}>
                                        <textarea type="text" className="inputComment" ref="commentInput"
                                    ...
                                    { this.state.comments.map((c_item, ic)=>
                                        <div className="activity-item" key={"comment-" + this.state.taskId + "-" + ic}>
                                            <div className="activity-avatar">
                                                <Avatar name={c_item["u.Name"]} avatar={c_item["u.Avatar"]}
                                                        color={c_item['ua.Color']}/>
                                            </div>
                        ...
                        <div className="col s4 bg-gray">
                            ...
                            <Dropdown trigger={
                                <div className="userAssigned">
                                    <div>Assigned to</div>
                                    <Avatar name={this.state.taskData['ua.Name']}
                                            avatar={this.state.taskData['ua.Avatar']}
                                            color={this.state.taskData['ua.Color']}/>
                            ...
                            <div className="rightBarItem">
                                ...
                                <Datetime isValidDate={this.validDateStart.bind(this)}
                                          onChange={this.selectStartDate.bind(this)}
                                          value={moment.unix(Math.round(parseInt(this.state.taskData['t.startDate']) / 1000))}/>
                                <strong>Due Date : </strong>
                                <Datetime isValidDate={this.validDateEnd.bind(this)}
                                          onChange={this.selectEndDate.bind(this)}
                                          value={moment.unix(Math.round(parseInt(this.state.taskData['t.endDate']) / 1000))}/>
                            </div>
                            <div className="rightBarItem" id="tags"
                                ...
                                <div>
                                    {this.state.currentTags.map((tag, i)=>
                                        <div key={"color-" + i} className={"tagColor " + tag["l.color"]} style={{
                                            backgroundColor: tag["l.bg_color"],
                                            color: tag["l.f_color"]
                                        }}>{tag["l.text"]}</div>
                                    )}
                                    <div className="clear"></div>
...

                                {
                                    ...
                                    <div id="tagList" className="fade-animation"
                                         onMouseOver={this._handleFocus.bind(this)}
                                         onMouseLeave={this._handleBlur.bind(this)}>
                                        <div id="btn-manage-tag" onClick={this.openTags.bind(this)}>[Manage Tags]</div>
                                        {this.state.allTags.map((taga, ti)=>
                                            <div className={this.classTag(taga['ID(t)'])}
                                                 key={"tag-all-" + ti + taga["ID(t)"]}
                                                 onClick={this.clickTag.bind(this, taga["ID(t)"], taga["t.color"], taga["t.text"], taga["t.bg_color"], taga["t.f_color"])}>
                                                <div className={"tagColor " + taga['t.color']} style={{
                                                    backgroundColor: taga['t.bg_color'],
                                                    color: taga['t.f_color']
                                                }}>{taga["t.text"]}</div>
                                                <div className="clear"></div>
                                            </div>
                                        )}
                ...
                {this.state.showTag ? <Tags projectId={this.state.projectId} socket={this.props.socket}
                                            closeTags={this.closeTags.bind(this)}/> : null}
            </div>
        );
    }
}
            ...
            <div style={{width: '450px'}}>
                <div style={{display: this.checkStatusActive()}}>
                    <button type="button" className="btn green"
                            onClick={this.props.changeStatus.bind(this, 'complete')}>Complete
                    </button>
